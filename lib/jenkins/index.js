...
var parse = function(headers, body, settings) {
  ...
  // success: phase = FINISHED, status = SUCCESS
  // failure: phase = FINISHED, status = FAILURE
  var build = body.build;

  // build.phase can be STARTED, COMPLETED or FINISHED
  var phase = build.phase.toLowerCase();

  // build.status only available for phases COMPLETED or FINISHED
  ...
  if(!events[phase] && !(phase === 'finished' && events[status])) return;
